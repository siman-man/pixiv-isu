...
module Isuconp
  class App < Sinatra::Base
    ...
    get '/' do
      ...
      results = db.query('SELECT `posts`.`id`, `user_id`, `body`, `posts`.`created_at`, `mime` FROM `posts` WHERE del_flg = 0 ORDER BY `posts`.`created_at` DESC LIMIT 20')
    ...
    get '/posts' do
      ...
      results = db.prepare('SELECT `posts`.`id`, `user_id`, `body`, `mime`, `posts`.`created_at` FROM `posts` WHERE `posts`.`created_at` <= ? AND del_flg = 0 ORDER BY `created_at` DESC LIMIT 20').execute(
